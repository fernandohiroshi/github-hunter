...
import type { GitHubUser, GitHubRepository, SearchState, SortOption } from '@/types/github'
  ...
  devtools(
    (set, get) => ({
      ...
      searchUser: async (username: string) => {
        ...
        const [userResult, reposResult] = await Promise.allSettled([fetchUser(trimmed), reposTask])
      ...
      loadMoreRepos: async () => {
        const { query, reposHasMore, reposNextPage, isLoadingRepos, isLoadingMoreRepos } = get()
        ...
        try {
          const perPage = 30
          const result = await fetchUserRepositoriesPage(query, reposNextPage, perPage)
...
export const selectRepositories = (state: SearchStore): GitHubRepository[] => state.repositories
export const selectIsLoading = (state: SearchStore): boolean =>
  state.isLoadingUser || state.isLoadingRepos
export const selectSortOption = (state: SearchStore): SortOption => state.sortOption
